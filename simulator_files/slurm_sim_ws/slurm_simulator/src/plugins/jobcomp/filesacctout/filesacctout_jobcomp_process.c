/*****************************************************************************\
 *  filesacctout_jobcomp_process.c - functions the processing of
 *                               information from the filesacctout jobcomp
 *                               database.
 *****************************************************************************
 *
...
typedef struct {
	char *name;
	char *val;
} filesacctout_jobcomp_info_t;


static void _destroy_filesacctout_jobcomp_info(void *object)
{
	filesacctout_jobcomp_info_t *jobcomp_info =
		(filesacctout_jobcomp_info_t *)object;
	if (jobcomp_info) {
		xfree(jobcomp_info);
	}
}
...
static jobcomp_job_rec_t *_parse_line(List job_info_list)
{
	...
	filesacctout_jobcomp_info_t *jobcomp_info = NULL;
...
extern List filesacctout_jobcomp_process_get_jobs(slurmdb_job_cond_t *job_cond)
{
	...
	while (fgets(line, BUFFER_SIZE, fd)) {
		...
		jobid = 0;
		partition = NULL;
		job_info_list = list_create(_destroy_filesacctout_jobcomp_info);
		while(fptr) {
			jobcomp_info =
				xmalloc(sizeof(filesacctout_jobcomp_info_t));
...
extern int filesacctout_jobcomp_process_archive(slurmdb_archive_cond_t *arch_cond)
{
